...
public class AttendanceService {
    ...
    public Map<String, Object> clockIn(AttendanceClockInDto dto) {
        employeeRepository.findByEmployeeDocId(dto.getEmployeeId())
    ...
    public Map<String, Object> clockOut(AttendanceClockOutDto dto) {
        AttendanceDocument doc = attendanceRepository.findByAttendanceId(dto.getRecordId())
    ...
    public Map<String, Object> getAttendanceSummary(String employeeId, int year, int month) {
        ...
        query.addCriteria(Criteria.where("date").regex("^" + monthPrefix));

        List<Map<String, Object>> records = mongoTemplate.find(query, AttendanceDocument.class)
                .stream().map(this::documentToMap).toList();
